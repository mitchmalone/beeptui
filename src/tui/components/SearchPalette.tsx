...
export function SearchPalette({ query, results }: SearchPaletteProps) {
  ...
  return (
    <box title="Search chats" border style={{ flexGrow: 1, flexDirection: 'column', padding: 1 }}>
      <text style={{ flexShrink: 0 }}>{`/ ${query}▏`}</text>
      <text style={{ flexShrink: 0, fg: '#94a3b8' }}>
        {results.length === 0 && query.length > 0 ? 'No matches' : 'Enter to jump · Esc to cancel'}
      </text>
      <box style={{ flexGrow: 1, flexDirection: 'column' }}>
        {shown.map((result, index) => (
          <text key={result.id} style={index === 0 ? { bg: '#334155', fg: '#ffffff' } : {}}>
