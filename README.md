...
brew install <owner>/tap/beeper-tui
beeper-tui doctor
...
#      chmod +x beeper-tui-darwin-arm64 && ./beeper-tui-darwin-arm64 doctor
...
runners and publishes a GitHub Release. A pending update (on branch
`feat/v1-polish`) also attaches `sha256sums.txt` and — when a tap is configured —
updates the formula; it lands once pushed with a `workflow`-scoped token. To
enable the tap, create a `homebrew-tap` repo, then set the repo variable
`HOMEBREW_TAP_REPO` (`owner/homebrew-tap`) and the secret `HOMEBREW_TAP_TOKEN` (a
token that can push to it). Without those, the release still publishes and the
tap step is skipped. The formula is generated by `src/packaging/homebrew.ts` (no
static file to hand-edit).
