...
configured — updates the formula. To enable the tap, create a
`homebrew-tap` repo, then set the repo variable `HOMEBREW_TAP_REPO`
(`owner/homebrew-tap`) and the secret `HOMEBREW_TAP_TOKEN` (a token that can push
to it). Without those, the release still publishes; the tap step is skipped. The
formula is generated by `src/packaging/homebrew.ts` (no static file to hand-edit).
